...
from static.constants import path_data
from templates.controllers.pmodel_controller import get_data_fichaje_db


def get_vector_absence_normal(value, total_days, total_value, year, month, day, data_out_x, id_emp=None):
    # day of the week
    dow = datetime(int(year), int(month), int(day)).weekday()
    if len(data_out_x) == 0:
        return [id_emp, dow, int(month), total_days, total_value, total_value, 0, total_value, 0]
    else:
        aux = np.array(data_out_x)
        return [id_emp, dow, int(month), total_days, total_value, np.mean(aux[:, 3]), np.std(aux[:, 3]),
...
def read_dict_and_vectorize(dic_data_absences, date_init, id_emp=None):
    """
        ...
        for month in range(1, 13):
            ...
            for day in days_of_month:
                if str(year) not in dic_data_absences.keys():
                    ...
                    total_value += 0.0
                    data_x.append(
                        get_vector_absence_normal(0.0, total_days, total_value, year, month, day, data_x, id_emp))
                    data_y.append([0, 0.0])
                    continue
                ...
                if str(day) not in dic_data_absences[str(year)][str(month)].keys():
                    ...
                    total_value += value
                    data_x.append(
                        get_vector_absence_normal(value, total_days, total_value, year, month, day, data_x, id_emp))
...
def get_basic_data_train_dict_absences(dic_data_absences: dict, date_init=None, id_emp=None) -> tuple:
    """
    ...
    dic_data_normal, data_x, data_y = read_dict_and_vectorize(dic_data_absences, date_init, id_emp)
    return data_x, data_y, dic_data_normal


def get_data_fichaje_train(date_init: str = None, filename: str = None):
    ...
    for item in result:
        ...
        data_x, data_y, data_dic_normal = get_basic_data_train_dict_absences(json.loads(absences), date_init, emp_id)
        data_train_x_a += data_x
        data_train_y_a += data_y
        data_x, data_y, data_dic_normal = get_basic_data_train_dict_absences(json.loads(lates), date_init, emp_id)
        data_train_x_l += data_x
        data_train_y_l += data_y
        data_x, data_y, data_dic_normal = get_basic_data_train_dict_absences(json.loads(extras), date_init, emp_id)
        data_train_x_e += data_x
        data_train_y_e += data_y
    columns = ("id_emp", "day_of_week", "month", "total_value", "mean", "std", "median", "var")
    data_train = {
        "absences": {"x": data_train_x_a, "y": data_train_y_a, "columns": columns},
        "lates": {"x": data_train_x_l, "y": data_train_y_l, "columns": columns},
        "extras": {"x": data_train_x_e, "y": data_train_y_e, "columns": columns}
    }
    # save data
    filename = filename if filename is not None else "data_train_absences.pkl"
    pathfile = path_data + filename + "pkl"
...
def get_data_fichaje_test_user(id_emp):
    ...
    emp_id, absences, lates, extras, primes, normal, date_admission = result if len(result) > 0 else [id_emp, {}, {},
                                                                                                      {}, {}, {}, ""]
    data_emp = get_basic_data_train_dict_absences(json.loads(absences))
    return data_emp


def extract_resume_data_dict(dic_data: dict):
    total_days = 0
    total_value = 0
    for year in dic_data.keys():
        for month in dic_data[year].keys():
...
def get_last_data_user():
    flag, error, result = get_data_fichaje_db()
    data_out = []
    for item in result:
        emp_id, absences, lates, extras, primes, normal, admission = item
        data_x, data_y, dic_data_normal = get_basic_data_train_dict_absences(json.loads(absences))
        data_out.append(data_x[-1])
        data_x, data_y, dic_data_normal = get_basic_data_train_dict_absences(json.loads(lates))
        data_out.append(data_x[-1])
        data_x, data_y, dic_data_normal = get_basic_data_train_dict_absences(json.loads(extras))
        data_out.append(data_x[-1])
        data_x, data_y, dic_data_normal = get_basic_data_train_dict_absences(json.loads(primes))
        data_out.append(data_x[-1])
